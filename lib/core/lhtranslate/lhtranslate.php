...
class erLhcoreClassTranslate
{
    ...
    public static function translateChatMessages(erLhcoreClassModelChat $chat)
    {
        ...
        if (isset($translationData['translation_handler']) && $translationData['translation_handler'] == 'bing') {
        ...
        } else if(isset($translationData['translation_handler']) && $translationData['translation_handler'] == 'baidu'){
            // Only last 10 messages are translated
            $msgs = erLhcoreClassModelmsg::getList(array(
                'filter' => array(
                    'chat_id' => $chat->id
            ...
            foreach ($msgs as $msg) {
                ...
                if ($msg->user_id != - 1) {
                    ...
                    if ($msg->user_id == 0) {
                        $msgTranslated = erLhcoreClassTranslateBaidu::translate($translationData['baidu_client_id'], $translationData['baidu_client_secret'], $msg->msg, $chat->chat_locale, $chat->chat_locale_to);
                    } else { // Operator message
                        $msgTranslated = erLhcoreClassTranslateBaidu::translate($translationData['baidu_client_id'], $translationData['baidu_client_secret'], $msg->msg, $chat->chat_locale_to, $chat->chat_locale);
                    }
    ...
    public static function translateTo($text, $translateFrom = false, $translateTo)
    {
        ...
        }elseif(isset($translationData['translation_handler']) && $translationData['translation_handler'] == 'baidu'){
            return erLhcoreClassTranslateBaidu::translate($translationData['baidu_client_id'], $translationData['baidu_client_secret'],$text, $translateFrom, $translateTo);
        }else {
