...
class erLhcoreClassGroupRole{
   ...
   public static function getGroupRoles($group_id)
   {
        ...
        $stmt = $db->prepare('SELECT lh_grouprole.id as assigned_id,lh_grouprole.role_id as roleid,lh_role.* FROM lh_role INNER JOIN lh_grouprole ON lh_grouprole.role_id = lh_role.id WHERE lh_grouprole.group_id = :group_id ORDER BY id ASC');
