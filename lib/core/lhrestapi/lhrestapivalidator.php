...
class erLhcoreClassRestAPIHandler
{
    ...
    public static function validateRequest()
    {
        ...
        if (isset($headers['Authorization'])) {
                ...
                if(strpos($headers['Authorization'], " ")){
                    $dataAuthorisation = explode(' ', $headers['Authorization']);
                }else{
                    $dataAuthorisation[1]  = $headers['Authorization'];
                }
   ...
   public static function unreadValidateChatList()
    {
        $validAttributes = array(
            'int' => array(
                'departament_id' => array(
        ...
        foreach($chats as $k=>$r){
            $unreadcount += $r->has_unread_messages;
              if($r->has_unread_messages) $id .= $r->id.',';
        }
        // Get chats count
        $chatsCount = erLhcoreClassChat::getCount($filter);
